...
const NodeRenderer = ({ node, data }: { node: MessageNode; data?: Record<string, any> }) => {
  const { type, properties = {}, children = [] } = node;

  const renderChildren = () =>
    children.map((child, idx) => <NodeRenderer key={child.id || idx} node={child} data={data} />);

  switch (type) {
    // --- Layout Components ---
    case 'Layout.Card':
      return <Card className={cn('p-4 space-y-4', properties.className)}>{renderChildren()}</Card>;
    case 'Layout.Stack':
      return <div className={cn('flex flex-col gap-4', properties.className)}>{renderChildren()}</div>;
    case 'Layout.Grid':
      return (
        <div
    ...
    case 'Display.Field':
      return (
        <div className={cn('flex flex-col gap-1', properties.className)}>
          <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{properties.label}</span>
          <div className="text-sm font-medium">{properties.value}</div>
        </div>
      );
    case 'Text.Paragraph':
      return <MessageRenderer content={properties.content} className={cn('text-sm', properties.className)} />;
    case 'Text.Heading':
      return <h4 className={cn('font-bold text-base', properties.className)}>{properties.content}</h4>;

    // --- Data Components ---
    case 'Data.StatsGrid':
      return <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-muted/30 rounded-lg">{renderChildren()}</div>;
    case 'Data.Stat':
      return (
    ...
    default:
      return (
        <div className="p-2 border border-dashed rounded text-xs text-muted-foreground">Unknown Component: {type}</div>
      );
  }
};
        ...
        <div className="p-4 border-b bg-card/50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {context.description && <p className="text-xs text-muted-foreground mt-1">{context.description}</p>}
            </div>
          </div>
