...
export function CommentRequestMessage({
  ...
  const [status, setStatus] = useState<'open' | 'resolved' | 'closed'>(message.metadata?.status || 'open');
      ...
      <div className="flex-1 min-w-0 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-sm text-foreground">{requester?.name}</span>
            <span className="text-muted-foreground text-xs">requested feedback</span>
        ...
        >
          <div className={cn('h-1 w-full', isResolved ? 'bg-green-500/40' : 'bg-amber-500')} />
            ...
            <div className="flex justify-between items-start gap-4">
              <div className="space-y-1.5 flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className={cn('font-medium text-sm', isResolved && 'line-through text-muted-foreground')}>
                ...
                */}
                <div className={cn('text-sm text-foreground/90 leading-relaxed pt-1', MARKDOWN_STYLES)}>
                ...
                {resourceMetadata.url && (
                  <div className="flex items-center gap-2 mt-3">
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-muted text-xs font-medium text-muted-foreground border border-border/50">
                      <span className="truncate max-w-[200px]">{resourceMetadata.url}</span>
                      {resourceMetadata.fileSize && <span className="opacity-50">| {resourceMetadata.fileSize}</span>}
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="relative flex gap-3 items-start">
              ...
              >
                <Textarea
                  ...
                  placeholder={isResolved ? 'This thread is resolved.' : 'Add a comment...'}
                  disabled={isResolved}
  ...
  return (
    <div className="flex gap-3 group/comment">
      <Avatar className="h-6 w-6 border border-border mt-0.5">
        ...
        <AvatarFallback className="text-[9px]">{author.name.substring(0, 2)}</AvatarFallback>
      </Avatar>

      <div className="flex-1 space-y-1">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-foreground/90">{author.name}</span>
