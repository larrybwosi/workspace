...
export function useInviteToWorkspace() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { data: response } = await apiClient.post(`/workspaces/${workspaceSlug}/invitations`, {
        userId,
        email,
        role,
      });
...
export function useAcceptWorkspaceInvitation() {
  ...
  return useMutation({
    mutationFn: async ({ workspaceSlug, invitationId }: { workspaceSlug: string; invitationId: string }) => {
      const { data: response } = await apiClient.patch(`/workspaces/${workspaceSlug}/invitations/${invitationId}`, {
        action: 'accept',
      });
      return response;
    },
