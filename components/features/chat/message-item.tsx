...
export function MessageItem({
  ...
  const customComponent = useMemo(() => {
    // If it's an implicit code message, we now route it to CustomMessage
    // CustomMessage has been enhanced to auto-generate a UI for these types,
    // including separating the text preamble from the code block.
    if (isImplicitCode) {
      return <CustomMessage message={message} readOnly />;
    }

    // Fallback to standard explicit custom types
  ...
  return (
    <ContextMenu>
            ...
            <div className="flex-1 min-w-0">
              ...
              {isEditing ? (
                <textarea
                  ...
                  className="text-sm leading-relaxed text-foreground border border-border rounded-lg bg-card p-2 w-full font-mono"
                  rows={5}
                />
              ) : (
                <>
                  {/* Hide standard markdown if we detected code to prevent duplication */}
                  {!isImplicitCode && (
                  ...
                  {/* Render the specialized component with auto-detected language */}
                  {customComponent}
                </>
              )}

              {/* ... (Attachments and Reactions remain unchanged) ... */}
      ...
      {/* ... (Context Menu Content remains unchanged) ... */}
