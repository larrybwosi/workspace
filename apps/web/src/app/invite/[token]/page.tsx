...
async function getInviteData(token: string) {
  ...
  if (inviteLink) {
    ...
    if (inviteLink.maxUses && inviteLink.maxUses > 0 && inviteLink.uses >= inviteLink.maxUses)
      return { error: 'Invite link has reached its use limit' };
    return {
      type: 'workspace_link',
      invitation: {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (type === 'platform_invitation') {
    return {
      title: `Invitation to join Scrymechat`,
      description: `${inviterName} has invited you to join Scrymechat.`,
    };
  } else {
    return {
      title: `Join ${invitation.workspace?.name} on Scrymechat`,
      description: `${inviterName} has invited you to join ${invitation.workspace?.name} on Scrymechat.`,
    };
  }
}
