  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      toast({
        title: 'Success!',
        description:
          initialInviteData.type === 'platform_invitation'
        ...
        <CardHeader className="text-center">
          <div className="flex justify-center mb-4">
            <Avatar className="h-20 w-20 border-2 border-background shadow-sm">
              <AvatarImage
                src={type === 'platform_invitation' ? invitation.inviter?.avatar : invitation.workspace?.icon}
              />
          ...
          <CardDescription>
            {type === 'platform_invitation' ? (
              <span>
                <span className="font-bold text-foreground">{inviterName}</span> invited you to join Scrymechat
              </span>
            ) : (
              <span>
                Join <span className="font-bold text-foreground">{invitation.workspace?.name}</span> on Scrymechat
              </span>
            )}
          </CardDescription>
        </CardHeader>
