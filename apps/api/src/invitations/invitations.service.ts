...
export class InvitationsService {
  ...
  async getInvitationByToken(token: string) {
    ...
    if (workspaceInvite) {
       ...
       */
      if (workspaceInvite.email && workspaceInvite.email.toLowerCase() !== user.email?.toLowerCase()) {
    ...
    if (generalInvite) {
       ...
       * Case-insensitive comparison prevents email format mismatch while blocking unauthorized users who obtain the token.
       */
      if (generalInvite.email && generalInvite.email.toLowerCase() !== user.email?.toLowerCase()) {
        throw new ForbiddenException('This invitation was sent to a different email address');
      }
