...
describe('IntegrationsService - GitHub integration (PR change)', () => {
  ...
  describe('handleGithubCallback (PR change)', () => {
    ...
    it('should throw NotFoundException when workspace is not found', async () => {
      ...
      await expect(service.handleGithubCallback('user-1', 'nonexistent-workspace', 'code123')).rejects.toThrow(
        NotFoundException
      );
    });
    ...
    it('should throw BadRequestException on GitHub OAuth error', async () => {
      ...
      const mockFetch = vi.fn().mockResolvedValueOnce({
        json: () =>
          Promise.resolve({ error: 'bad_verification_code', error_description: 'The code passed is incorrect' }),
      });
      global.fetch = mockFetch as any;

      await expect(service.handleGithubCallback('user-1', 'my-workspace', 'bad_code')).rejects.toThrow(
