...
export class ApiTokensController {
  ...
  async getApiTokens(@CurrentUser() user: User, @Param('slug') slug: string) {
     ...
     * 1. Consolidates workspace lookup, membership verification, and API token retrieval into a single database query.
     * 2. Reduces database round-trips from 3 down to 1.
     * 3. Uses 'select' to fetch only essential fields.
     */
    ...
    const tokens = workspace.apiTokens;
