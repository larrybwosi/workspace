...
import { extractUserMentions, extractChannelMentions, hasSpecialMention, extractUserIds } from './mention-utils';
...
describe('extractUserMentions', () => {
  ...
  it('should extract multiple mentions including dots', () => {
    expect(extractUserMentions('@alice and @bob.smith are here')).toEqual(['alice', 'bob.smith']);
  });
...
describe('extractUserIds', () => {
  ...
  it('should handle username containing dots correctly (PR change)', () => {
    ...
    const dotUsers = [{ id: 'user-dot', name: 'Dot User', username: 'first.last' }];
