    ...
    <View className="bg-discord-tertiary rounded-lg border border-black/20 my-2 overflow-hidden">
      ...
      <View className="p-4">
        <View className="bg-discord-base p-3 rounded-lg border border-black/10 mb-4">
          ...
          <Text className="text-discord-header text-sm leading-5">
            {reportData.summary || message.content}
          </Text>
        </View>
        ...
        {reportData.kpis && (
          <View className="flex-row flex-wrap gap-2 mb-4">
             {reportData.kpis.slice(0, 2).map((kpi: any, idx: number) => (
               <View key={idx} className="bg-discord-secondary p-3 rounded-lg border border-black/10 flex-1 min-w-[120px]">
                  <Text className="text-discord-muted text-[10px] font-bold uppercase mb-1">{kpi.label}</Text>
                  <Text className="text-discord-header font-bold text-lg">{kpi.value}{kpi.unit}</Text>
               </View>
             ))}
          </View>
        )}
